...
## 2026-09-20: Genetic algorithm design

- **One generation is one run.** Each Folk in the run is one individual (its decider's parameter array is its genome), alone at a random place in a shared world, no replacement. The run lasts 50,000 ticks or stops early when fewer than 25% are alive. Fitness is ticks lived, plus up to 1,000 for the calorie reserve left by Folk still alive, so survivors are ranked by how well off they are. Each generation uses a new world seed so the parameters cannot overfit one map.
- **Breeding:** the best 5% are copied unchanged; the rest are children of two tournament-picked parents (best of 3), each parameter taken from either parent, then changed with 20% chance by a small normal step (10% of the parameter's range), kept in range. All of it is in `packages/sim/src/evolution.ts` and seeded, so runs repeat.
- **Curriculum:** food coverage starts at 1x. After each generation, if more than 55% of Folk survived, coverage is multiplied by 0.92; if fewer than 45%, by 1.04; otherwise unchanged. This holds survival near the 50% target while the Folk get better, so the coverage reached is the measure of progress.
- **Folk count and world size were chosen by measurement.** Cost per tick is linear in Folk (about 2.5 microseconds each), so time is not the limit; crowding is. Utility survival at 0.5x food fell from 94% (50 Folk on a 256 by 256 world) to 88% (100), 62% (200) and 39% (300), so density above about 100 per 256 by 256 tiles makes Folk compete for the same patches. On a 512 by 512 world, 400 Folk (the same density) gave 94%, matching the uncrowded case, at about 1 ms per tick (about a minute for a full generation); 800 Folk gave 70%. So the default is 400 Folk on 512 by 512. Bigger populations need a bigger world.
- **First results** (100 generations each, 400 Folk, 512 by 512 world, seed 1; `experiments/output/evolve-*-run1`): the curriculum held survival near 50% while lowering food, reaching **0.12x coverage for utility and 0.17x for rules** (mean of the last 30 generations; the untrained figures were 47% and 94% survival at 0.25x, and 50% survival needs below 0.25x even for rules). Utility, the weaker decider before evolution, ends up ahead of rules. Evolved utility parameters hit the edges of their ranges (`wYield` at its maximum of 2, `wEffort` and `wWander` at 0), so the ranges may be limiting; widening them and running more seeds is the next step. No generation stopped early.

## 2026-09-21: Overnight evolution batch

- **Why wider ranges:** the first run pushed several utility parameters to the edge of their range (`wYield` at 2, `wEffort` and `wWander` at 0) and rules ones too (`eatBelow` 0.7, `foodTarget` 12000, `restIfInjured` 0), so the default ranges may be capping the result. `configs/wide-params.json` opens them up (weights 0 to 5, memory half-life 100 to 100,000 ticks, and so on); the comparison against the default ranges tells whether that matters.
- **Batch layout:** decider x range ("default" or "wide") x seeds 1, 1001 and 2001 (far apart because generation g uses world seed SEED + g, so nearby seeds would share worlds), 150 generations each, four runs at a time. A run is scored by the food coverage it holds while about half its Folk survive (mean over its last 30 generations; lower is better). Wide ranges start with worse random Folk (in a small test, utility survival at full food began around 40%), so early generations can be slow.
