...
What it answers, for tuning the world and the behaviours:
  ...
  energy_by_action    where each decider's Folk spend their energy
  food_sources        where satiety comes from: species, terrain, success rate
...
def energy_by_action(run: Run) -> pd.DataFrame:
    """Energy each decider's Folk spent on each action, and the share of the total."""
    last = _final(run.csv("activity.csv")).copy()
    last["share"] = last["energySpent"] / last.groupby("decider")["energySpent"].transform("sum")
    return last[["decider", "action", "energySpent", "energyGained", "share"]]
...
def food_sources(run: Run) -> pd.DataFrame:
    ...
    df["satiety_share"] = df["satiety"] / df.groupby("decider")["satiety"].transform("sum")
...
def lifetimes(run: Run) -> pd.DataFrame:
    ...
    df["satiety_per_tick"] = df["satietyEaten"] / df["lived"].clip(lower=1)
    df["energy_per_tick"] = df["energySpent"] / df["lived"].clip(lower=1)
    df["steps_per_tick"] = df["steps"] / df["lived"].clip(lower=1)
    return df


def parameter_effects(run: Run) -> pd.DataFrame:
    """Correlation of each decider parameter with satiety eaten per tick lived (a GA fitness proxy)."""
    df = lifetimes(run)
    out = []
        ...
        for i, p in enumerate(decider["params"]):
            ...
            if len(mine) > 3 and mine[col].nunique() > 1:
                out.append({
                    ...
                    "corr_satiety_per_tick": mine[col].corr(mine["satiety_per_tick"]),
                    "corr_energy_per_tick": mine[col].corr(mine["energy_per_tick"]),
                })
...
def plot_energy_by_action(run: Run) -> plt.Figure:
    return _stacked(energy_by_action(run), "energySpent", "Energy spent by action", ACTION_COLORS)
...
def plot_food_sources(run: Run) -> plt.Figure:
    ...
    for j, decider in enumerate(deciders):
        ...
        by_species = mine.groupby("species")["satiety"].sum()
        axes[0][j].bar(by_species.index, by_species.values, color=[SPECIES_COLORS.get(s) for s in by_species.index])
        axes[0][j].set(title=f"{decider}: satiety by species")
        by_terrain = mine.groupby("terrain")["satiety"].sum().sort_values(ascending=False)
        axes[1][j].bar(by_terrain.index, by_terrain.values, color=[TERRAIN_COLORS.get(t) for t in by_terrain.index])
        axes[1][j].set(title=f"{decider}: satiety by terrain")
    fig.tight_layout()
    return fig
...
def plot_lifetimes(run: Run) -> plt.Figure:
    df = lifetimes(run)
    metrics = [("satiety_per_tick", "satiety eaten / tick"), ("energy_per_tick", "energy spent / tick"),
...
PLOTS = {
    ...
    "energy_by_action": plot_energy_by_action,
...
def report(run: Run, out: Path | None = None) -> Path:
    ...
    print("\n== energy by action (share of energy spent) ==")
    print(energy_by_action(run).pivot(index="action", columns="decider", values="share"))
    print("\n== where satiety comes from (share, by decider and species) ==")
    src = food_sources(run)
    print(src.groupby(["decider", "species"])[["attempts", "successes", "units", "satiety"]].sum())
    print("\n== where satiety comes from (by terrain) ==")
    print(src.groupby(["decider", "terrain"])["satiety"].sum().unstack(0))
    ...
    print(life.groupby("decider")[["lived", "meals", "satiety_per_tick", "energy_per_tick", "injuries"]].mean())
    effects = parameter_effects(run)
    if len(effects):
        print("\n== parameter correlation with satiety eaten per tick ==")
